...
| HR-A（本轮已完成安全闭环） | Tool runtime owner migration | 已把 provider-neutral file guidance marker、file-read freshness comparison、oversized tool-result preview/rendering policy 迁入 `bitfun-agent-tools`；本轮进一步把 `ToolUseContext` 本体、portable facts projection、runtime/service binding、unified `Tool::call` runtime hook facade 和 Deep Review post-call hook regression 收敛到 core `tool_context_runtime.rs`，`framework.rs` 只保留 `Tool` trait 与旧路径 re-export，core runtime/adapter 模块直接引用 owner 路径；core 继续持有 Read/Edit/Write concrete IO、session read-state storage、tool-result filesystem persistence、manifest execution、snapshot wrapper 与 collapsed unlock state | product-domain runtime、remote/service runtime、H5 feature matrix、未设计 port/provider 的 concrete tool/runtime owner 外移 | owner-crate contract tests、ToolUseContext facts/runtime focused tests、Read/Edit/Write guardrail focused tests、tool-result storage focused tests、boundary check 与 product-full check 通过；不得声明完整 tool runtime 已迁出 core |
...
host adapters, remote model catalog/session-model selection config adapters,
remote chat history persistence/message conversion adapters, remote image-context
conversion, core-owned workspace-root lookup helper, and `ConversationCoordinator`
runtime-port adapter binding. The model/history adapter moves only centralize existing
core reads and persisted-turn-to-mobile-message conversion for remote catalog/session
selection and chat history; they do not move remote-SSH runtime, workspace-root source
ownership, broad persistence/workspace service ownership, concrete scheduler/session restore/terminal adapters,
