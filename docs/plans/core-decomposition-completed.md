...
- `services-core` 已承接 session storage file layout、目录创建 helper、turn file indexing/deletion、metadata store CRUD/index rebuild IO、metadata pagination、metadata construction/counter/index/field mutation、lineage / branch metadata shaping 规则和通用 JSON file store；core persistence manager 保留 workspace-to-sessions-root 解析、session state/turn/prompt-cache IO orchestration、legacy workspace-runtime metadata merge 兼容处理和产品 runtime 接线。
