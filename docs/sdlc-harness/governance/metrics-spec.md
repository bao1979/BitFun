...
- 核心体验指标必须按用户画像、任务风险、内部策略画像、用户可见投影、入口平台和受管状态切片，避免平均值掩盖局部人群摩擦。
...
| 用户打断率 | 每个任务的确认、提示、阻断次数 | 产品体验 | `security.decided`、`policy.decided`、用户界面事件 | 每周 | 降低提示噪音 |
| 弹窗触发率 | 模态确认或阻断次数 / 任务数 | 产品体验 | 用户界面事件、`security.decided` | 每周 | 防止确认打断泛化 |
...
| 强制检查漏报率 | 后验发现应运行但未推荐的检查 / 后验确认需要的检查总数 | 风险分类器 | CI 失败、审查阻塞项、合入后缺陷 | 每两周 | 降低错误就绪风险 |
...
| P0 -> P1 | 首次有用动作耗时、打断率、安全提示响应、信心摘要覆盖率和关键用例验收覆盖率有基线 |
