...
Kernel 负责状态和副作用，插件适配主机负责插件语言运行时和生态 API 适配。适配主机崩溃、重启、超时或返回旧结果时，Kernel 必须能降级、拒绝或重放事件。
...
| 等级 | 适用 | 要求 |
|---|---|---|
| cell | 受信任观察/建议插件 | 独立模块图、事件订阅和超时；不得访问真实 shell、网络、凭据 |
| worker | 建议、guard、轻量自定义工具 | 独立工作线程或等价隔离；所有副作用走 Kernel facade |
| subprocess | 工具复写、act、高风险依赖 | 独立进程、资源预算、环境变量白名单、工作目录限制 |
| sandbox | 未知来源或企业受管场景 | 容器、无凭据环境、网络策略和只读/临时 worktree |
