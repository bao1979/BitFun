...
| 生态扩展预留 | 先稳定 Kernel 扩展点；OpenCode 等插件生态后续接入时必须在 BitFun 边界内生效 |
...
| 技术负责人 / 架构师 | 关键变更展示影响候选、验证缺口和风险接受入口 | API、数据、权限或架构边界变化 | 影响候选、证据引用和风险接受分离 |
...
| 团队治理 | 团队配置、受保护分支、路径规则、PR 协作 | PR 就绪度、审查建议、风险接受入口 | 按团队配置投影证据引用和检查要求 |
...
| Kernel 扩展点 | 生命周期 hook、事件、打点、工具复写候选和插件效果候选 | 先提供空实现和稳定契约；外部插件运行时不作为默认能力 |
...
| 安全确认面板 | 展示命令/文件/域名/凭据、授权范围、后果、允许一次/会话/拒绝/隔离选项 | 仅安全动作触发 | shell、网络、凭据、删除、跨目录写、发布 |
...
| 设置页扩展 | 提供提示密度、默认授权范围、团队规则来源、应急放行记录和插件契约开关 | 不打断主流程 | 用户主动进入设置或审计场景 |
...
| 准备 PR 或分享变更 | 中到高 | 用户准备提交 PR、复制结果，或检测到受保护分支/团队策略 | 按需显示 PR 就绪度面板，列出变更、验证、未验证、风险和证据引用 | 减少审查人追问，用户不用手动整理验证和残余风险 | 就绪度不在普通任务中间过程显露；PR 摘要可复制 |
...
| 插件兼容与核心模型 | OpenCode 等生态后续通过适配器接入；BitFun 内部只依赖规范事件、权限和插件效果候选契约 |
...
| PRD-03 | 配置化策略决定内部策略画像 | 内部画像带原因、来源、展示层级和覆盖选项，并投影为用户可理解的状态、提示或确认 |
...
| PRD-16 | Kernel 扩展点预留 | 生命周期 hook、事件、打点、工具复写候选和插件效果候选有稳定契约，且空实现不影响默认任务 |
...
| 阶段 | 产品重点 |
|---|---|
| P0 | 快速路径、轻量项目理解、低噪音安全边界 |
| P1 | 风险解释、弱提示降噪、推荐检查、PR 就绪度摘要、远程开发基础适配 |
| P2 | 团队配置、路径规则、强制检查、风险接受和主动配置信任 |
| P3 | 需求、PR、发布、事故和回归资产追溯 |
| P4 | 真实任务回放、策略评测、指标校准、保留集和生态适配评估 |
