...
| `bitfun-product-domains` | Miniapp 和 function-agent 产品子域 | partial：pure decision、port、MiniApp create/update/draft/apply/import state transition、storage/builtin contract、imported meta timestamp policy 与 seed meta timestamp policy 可迁入；IO、worker、built-in asset seeding、Git/AI service runtime 仍在 core |
...
- 后续 runtime 迁移以 `docs/plans/core-decomposition-plan.md` 的里程碑表为准，不再按零散
  ...
  - H1：tool runtime owner 迁移。当前只完成迁移前/迁移中边界收敛：`bitfun-agent-tools`
    ...
    收敛到 provider-neutral `bitfun-agent-tools`，
